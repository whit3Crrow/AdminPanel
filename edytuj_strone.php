...
<body>
    ...
    <div class="container myboxshadow rounded-3 border my-4 p-3">
        <?php 
         ...
         $stmt = $pdo->prepare("SELECT * FROM `strony` WHERE id = ?");
         $stmt->bindParam(1, $id, PDO::PARAM_INT);
         $stmt->execute();
         $result = $stmt->fetch(PDO::FETCH_ASSOC);

        echo("
        <form class='d-flex flex-column' action='send_edytujstrona.php' method='POST'>
            <label class='lead fs-3'>Tytuł Strony w Nawigacji</label>
            <input class='m-2 border' placeholder='Skrót' name='nav'  value='{$result['nav']}'></input>
            <label class='lead fs-3'>Nagłówek (wstęp)</label>
            <input class='m-2 border' placeholder='Nagłówek' name='tytul' value='{$result['tytul']}'></input>
            <input name='id' type='hidden' value='{$id}'></input>
            <input name='old' type='hidden' value='{$result['nav']}'></input>
