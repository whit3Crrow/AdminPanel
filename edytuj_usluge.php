...
<body>
    <header class="d-flex justify-content-center bg-dark text-white-50 p-2 display-1 ">Edytowanie Usługi</header>
    <div class="m-4"></div>
    ...
    <div class="container myboxshadow rounded-3 border my-4 p-3">
        <?php
            ...
            $stmt = $pdo->prepare("SELECT * FROM `usługi` WHERE id = ?");
            $stmt->bindParam(1, $id, PDO::PARAM_INT);
            $stmt->execute();
            $result = $stmt->fetch(PDO::FETCH_ASSOC);

            echo("
                <form class='d-flex flex-column' method='POST' enctype='multipart/form-data' action='send_edytujusluge.php' >
                    <label class='lead fs-3'>Tytuł usługi</label>
                    <input class='m-2 border' name='title' placeholder='Nagłówek' value='{$result['tytul']}' ></input>
                    <label class='lead fs-3'>Krótkie streszczenie usługi</label>
                    <input class='m-2 border' name='summary' placeholder='Opis' value='{$result['opis']}'></input>
                    <input name='id' type='hidden' value='{$id}'></input>
                    <input name='old' type='hidden' value='{$result['tytul']}'></input>
                    ...
                    <button type='submit' class='btn btn-outline-success myboxshadow p-2 w-100'>Potwierdź Edycję</button>
                </form>");
            ?>
    </div>
</body>
